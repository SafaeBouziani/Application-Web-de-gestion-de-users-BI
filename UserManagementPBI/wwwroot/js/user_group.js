    ...
    var CreateUserModal = {
        ...
        init: function () {
            ...
            $modal.on('click', '#btn-add-quick-group', function () {
                ...
                if (!title || !title.trim()) { alert('Entrez un titre de groupe'); return; }
        ...
        renderQuickNewGroupsList: function () {
            ...
            this.state.newGroups.forEach(function (g, idx) {
                ...
                $el.append('<button class="btn btn-sm btn-link quick-group-remove" data-index="' + idx + '">Supprimer</button>');
                $list.append($el);
            });
        },
        ...
        renderNewGroupsPanels: function () {
            ...
            var self = this;
            if (!this.state.newGroups.length) {
                container.append('<div class="text-muted">Aucun groupe ajouté. Utilisez "Ajouter nouveau groupe" ou le quick-add dans l\'étape 2.</div>');
                return;
            }
        ...
        renderGroupReports: function (gidx) {
            ...
            g.reports.forEach(function (r, ridx) {
                ...
                if (window.__catalogMap && r.Id && window.__catalogMap[r.Id]) {
                    pathText = "Chemin: /" + window.__catalogMap[r.Id].Path;
                    pathDisplay = 'block';
                }
                var $row = $(`
        ...
        renderSummary: function () {
            ...
            if (s.selectedExistingGroupIds && s.selectedExistingGroupIds.length) {
                ...
                $eg.append('<div class="text-muted">Aucun</div>');
            }
            $c.append($eg);
            ...
            if (s.newGroups.length) {
                s.newGroups.forEach(function (g, gi) {
                    ...
                    $gdiv.append('<div><strong>' + (g.title || '(Sans titre)') + '</strong></div>');
                    $gdiv.append('<div>' + (g.comment || '') + '</div>');
                    if (g.reports && g.reports.length) {
                        var $rul = $('<ul></ul>');
                        g.reports.forEach(function (r) {
                            $rul.append('<li>' + (r.Title || '(Sans titre)') + ' — id:' + (r.Id || '') + ' — ordre:' + (r.Order || '') + '</li>');
                        });
                        $gdiv.append($rul);
                    } else {
                        $gdiv.append('<div class="text-muted">Pas de rapports</div>');
                    }
                    $ng.append($gdiv);
                });
            } else {
                $ng.append('<div class="text-muted">Pas de nouveaux groupes</div>');
            }
            $c.append($ng);
        },
        ...
        submit: function () {
            var self = this;
            var userId = parseInt($('#assign-group-form input[name="Id"]').val() || "0");
            ...
            // anti-forgery token
            var token = $('#assign-group-form input[name="__RequestVerificationToken"]').val();
            var url = '/Users/AssignGroup/' + userId   // Edit
            ...
            $.ajax({
                ...
                success: function (resp) {
                    if (resp && resp.success) {
                        ...
                        // small success feedback then reload index to reflect created user
                        alert('Groupe Assigné avec succès. La page va se raffraîchir.');
                        location.reload();
                    } else {
                        alert('Erreur serveur: ' + (resp && resp.message ? resp.message : 'inconnue'));
                    }
                },
                error: function (xhr) {
                    var info = 'Une erreur est survenue';
                    try { info = JSON.parse(xhr.responseText).message || xhr.responseText; } catch (e) { info = xhr.responseText; }
                    alert('Enregistrement echoué: ' + info);
                }
            });
        }
    };
